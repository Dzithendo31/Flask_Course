...
#Now to acces the url from the .env file
#Install package python .env package
from dotenv import load_dotenv
load_dotenv()#laoding to our environmental variables.

#Import the os package
import os

contection_string = os.environ.get("AZURE_DATABASE_URL")
app.config["SQLALCHEMY_DATABASE_URL"] = connection_string
db = SQLALCHEMY(app)


#Method to use    db.session.delete(movie) -----after you finf the actuall movie
...
@app.get('/movies/<id>')#Id is now a varibles
def get_movie(id):
    movie = Movie.query,get(id) #now we actually quering the data.
    ...
    if movie == None:
        ...
        data = movie.to_dict() # to conver it to a dictionary
        #Still the same to make sure that the exceptio of being none is Done
        return jsonify(movie),200
# @app.get("/movies")
# def get_movies():
...
##Taks Now use the data that coming from Assure other than the 
#Goi to the NExt line
@app.route("/movie-list/<id>")
def movie_list_id():
    return render_template("movie_list.html",movies=movies)
    #Now chnage to use the data from the database instead of the list of dictionaries
    return render_template("movie_list.html",movies=movies)
@app.route("/movies/<id>")
